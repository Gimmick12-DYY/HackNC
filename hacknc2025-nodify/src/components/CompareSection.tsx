...
export default function CompareSection({ info }: Props) {
  const groupedRelations = React.useMemo(() => {
    ...
    // Put root first if it has children
    ...
    >
      <div className="h-full flex flex-col">
        <div className="p-4 border-b border-[#e6dccb] bg-gradient-to-r from-[#f7f2e8] to-[#f3eadb]">
          ...
          <p className="text-sm text-[#6b7280] mt-1">Right-click a node → Get info</p>
        </div>
          ...
          {!info ? (
            ...
            <div className="space-y-4">
              <div className="bg-white border border-[#e5e7eb] rounded-lg p-3">
                <div className="text-xs uppercase tracking-wide text-[#6b7280] mb-1">Root</div>
                <div className="text-sm text-[#171717] break-words">{info.nodes[info.rootId]?.text || ""}</div>
              </div>
              <div className="bg-white border border-[#e5e7eb] rounded-lg p-3">
                <div className="text-xs uppercase tracking-wide text-[#6b7280] mb-2">Subnodes</div>
                {(() => {
                  ...
                  <div className="text-sm text-[#6b7280]">No subnodes.</div>
                    ...
                    <ul className="space-y-2">
                      {directChildren.map(n => (
                        <li key={n.id} className="border border-[#e5e7eb] rounded-md p-2 bg-[#f9fafb]">
                          <div className="text-[13px] text-[#111827] break-words">{n.text || "(empty)"}</div>
                          {n.children.length > 0 && (
                            <div className="mt-1 text-[12px] text-[#6b7280]">Children: {n.children.length}</div>
                          )}
                        </li>
                      ))}
                    </ul>
              ...
              <div className="bg-white border border-[#e5e7eb] rounded-lg p-3">
                <div className="text-xs uppercase tracking-wide text-[#6b7280] mb-2">Relationships</div>
